...
class Seccion:
    ...
    def crear(cls, numero, instancia_id, profesor_id=None, sala_id=None):
        q = "INSERT INTO secciones (numero, instancia_id, profesor_id, sala_id) VALUES (?,?,?,?)"
    ...
    def obtener_por_id(cls, sid):
        r = execute_query("SELECT id,numero,instancia_id,profesor_id,sala_id FROM secciones WHERE id=?",(sid,))
        if r:
            a=r[0]
    ...
    def actualizar(self):
        q="UPDATE secciones SET numero=?,instancia_id=?,profesor_id=?,sala_id=? WHERE id=?"
        execute_query(q,(self.numero,self.instancia_id,self.profesor_id,self.sala_id,self.id))
        return True

    @staticmethod
    def asignar_sala(seccion_id, sala_id):
        execute_query("UPDATE secciones SET sala_id=? WHERE id=?",(sala_id,seccion_id))

    @staticmethod
    def eliminar(sid):
        execute_query("DELETE FROM secciones WHERE id=?",(sid,))
    ...
    def obtener_profesores_disponibles(instancia_id, seccion_id_actual=None):
        ...
        q="SELECT DISTINCT profesor_id FROM secciones WHERE instancia_id=? AND profesor_id IS NOT NULL"
        params=[instancia_id]
